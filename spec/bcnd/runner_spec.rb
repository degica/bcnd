  ...
  describe "#deploy" do
    context "when master branch" do
      ...
      it "wait for quay build to be finished" do
        ...
        expect(runner).to receive(:system).with("bcn deploy -e staging --tag aaaaaa --heritage-token mainline_heritage_token")
    ...
    context "when production branch" do
      ...
      it do
        ...
        runner = described_class.new
        expect(runner).to receive(:system).with("bcn deploy -e production --tag aaaaaa --heritage-token stable_heritage_token")
        expect{runner.deploy}.to_not raise_error
      end
    end
  end
